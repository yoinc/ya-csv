...
CsvWriter.prototype.writeRecords = function(recs) {
    if (!recs) return; // ignore empty records
    if (!Array.isArray(recs)) {
        throw new Error("CsvWriter.writeRecord only takes an array as an argument");
    }
    for (var i = recs.length - 1; i >= 0; i--) {
        _writeArray(this, recs[i], i != 0);
    };
};
...
CsvWriter.prototype.writeRecord = function(rec) {
    ...
    _writeArray(this, rec, false);
};

var out = '';

function _writeArray(writer, arr, wait) {
    for (var i = 0; i < arr.length; i++) {
        if (i != 0) out += writer.separator;
        out += writer.quotechar;
        out = _appendField(out, writer, arr[i]);
        out += writer.quotechar;
    }
    out += "\r\n";
    if (!wait) {
        writer.writeStream.write(out, this.encoding);
        out = '';
    }
};
...
function _appendField(outArr, writer, field) {
    ...
    for (var i = 0; i < field.length; i++) {
        ...
        if (c === writer.quotechar || c === writer.escapechar) {
            outArr += writer.escapechar;
        }
        outArr += c;
    }
    return outArr;
